      ...
      socket.onmessage = (event) => {
        ...
        try {
          ...
          if (message.type === "portfolio-update") {
            setStatus("resyncing");
            Promise.resolve(resyncRef.current?.())
              .then(() => {
                if (generation === socketGeneration && !isUnmountedRef.current) {
                  setStatus("live");
                }
              ...
              .catch(() => {
                if (generation === socketGeneration && !isUnmountedRef.current) {
                  setError("Portfolio state could not be refreshed after an automatic exit.");
          ...
          // The typed feed parser below handles malformed frames.
        }
